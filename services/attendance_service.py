...
def get_org_performance_analytics(department: Optional[str] = None) -> Dict:
    ...
    manager_averages.sort(key=lambda x: x["score"], reverse=True)
...
def check_and_send_absence_warnings():
    """
    Evaluates attendance for all active interns. If an intern has 3 consecutive
    days of unapproved absence, sends a warning email and logs it to prevent
    duplicate emails for the same streak.
    """
