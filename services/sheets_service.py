...
──────────────────────────
Single entry point for ALL Google Sheets operations.
Never scatter gspread calls across route files — always go through here.

Sheet names (worksheet tabs):
  Tasks | Submissions | Attendance | Warnings | Leaves | Reports | Performance
"""
...
_PERFORMANCE_COLS = {
    "report_id": 0, "intern_id": 1, "manager_id": 2, "week_start": 3, "week_end": 4,
    "work_quality": 5, "task_completion": 6, "learning_ability": 7, "teamwork": 8,
    "discipline": 9, "behaviour": 10, "overall": 11, "total_score": 12, "grade_band": 13,
    "strengths": 14, "areas_improvement": 15, "overall_comments": 16, "submitted_at": 17,
}
_INVITES_COLS = {
...
SHEET_HEADERS = {
    ...
    "Performance": ["report_id", "intern_id", "manager_id", "week_start", "week_end", "work_quality", "task_completion", "learning_ability", "teamwork", "discipline", "behaviour", "overall", "total_score", "grade_band", "strengths", "areas_improvement", "overall_comments", "submitted_at"],
...
def create_performance_report(intern_id: str, manager_id: str, week_start: str, week_end: str,
                              work_quality: int, task_completion: int, learning_ability: int,
                              teamwork: int, discipline: int, behaviour: int, overall: int,
                              total_score: int, grade_band: str, strengths: str, 
                              areas_improvement: str, overall_comments: str) -> Dict:
    sheet = _get_sheet("Performance")
    report_id = _new_id()
    created_at = _now_utc_str()
    row = [
        report_id, intern_id, manager_id, week_start, week_end,
        work_quality, task_completion, learning_ability, teamwork,
        discipline, behaviour, overall, total_score, grade_band,
        strengths.strip(), areas_improvement.strip(), overall_comments.strip(), created_at
    ]
    sheet.append_row(row, value_input_option="RAW")
