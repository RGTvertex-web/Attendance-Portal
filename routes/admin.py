...
def dashboard():
    ...
    recent_warnings = sorted(ss.get_all_warnings(), key=lambda w: w["date"], reverse=True)[:10]
...
def export_data(report_type, format):
    ...
    if report_type == "performance":
        ...
        data = ss.get_attendance_for_date(datetime.now(timezone.utc).strftime("%Y-%m-%d"))
